...
class WearProjectionTest {
    ...
    fun `the current set and its target cross over`() {
        val state = snapshot(currentSetIndex = 1).toWearState(NAMES)!!
    ...
    fun `the revision crosses over`() {
        assertEquals(9L, snapshot(revision = 9).toWearState(NAMES)!!.revision)
    }
    ...
    fun `a timed exercise reports no rep target`() {
        ...
        assertNull(timed.toWearState(NAMES)!!.targetReps)
    }
    ...
    fun `the rest deadline crosses over untouched`() {
        ...
        assertEquals(123_456L, resting.toWearState(NAMES)!!.deadlineElapsedRealtimeMs)
    }
    ...
    fun `the next exercise is named while there is one`() {
        assertEquals(
            ...
            snapshot().toWearState(NAMES)!!.nextExerciseName,
        )
    }
    ...
    fun `the last exercise has nothing after it`() {
        ...
        assertNull(onTheLast.toWearState(NAMES)!!.nextExerciseName)
    }
    ...
    fun `an unresolvable name falls back to the id`() {
        assertEquals("0025", snapshot().toWearState(emptyMap())!!.exerciseName)
    }

    /** §11's "no workout" screen is the absence of a snapshot, not a quiet one. */
    @Test
    fun `an idle session publishes nothing`() {
        assertNull(snapshot(phase = SessionPhase.IDLE).toWearState(NAMES))
    }

    @Test
    fun `a session with no exercises publishes nothing`() {
        assertNull(snapshot(exercises = emptyList()).toWearState(NAMES))
    }
    ...
    fun `every phase the watch can be in maps to one of its own`() {
        ...
        val actual = mapped.keys.associateWith { snapshot(phase = it).toWearState(NAMES)?.phase }
    ...
    fun `an abandoned workout is not reported as finished`() {
        val abandoned = snapshot(phase = SessionPhase.ABANDONED).toWearState(NAMES)!!
