...
### 2026-09-15 - the key moved into the tree, and the README got a face

**The signing key lives at `.repforth/` in the working tree now**, at the
owner's request, rather than in the home directory. The trade is real and worth
naming: the repository is public, so being ignored stopped being a convenience
and became the only thing between the key and a `git add -A`.

Three things enforce it rather than one, because a key committed once has to be
replaced and every install orphaned:

- `/.repforth/` in the root `.gitignore`, alongside the existing `*.jks` and
  `signing.properties` rules.
- `.repforth/.gitignore` containing `*`, which excludes the folder's contents
  from inside it and therefore survives an edit to the root file.
- `requireIgnoredByGit` in `ReleaseSigning.kt`, which asks `git check-ignore`
  and **fails the build rather than signing with a key git can see**. Watched
  failing: both ignore rules were stripped and the build refused by name.

`providers.exec` rather than `ProcessBuilder` for that check, and not by
preference. The configuration cache rejects an external process started during
configuration, with four problems reported and the build failed - which is the
right answer, since a build whose result depends on an unrecorded command cannot
be replayed. Through Gradle the call is an input like any other.

**The README has screenshots**, generated by `tools/readme-shots.py` from the
app's own goldens: trimmed of the dead space under short screens, corner-masked,
halved in resolution, and round-masked for the watch.

**Rendered rather than photographed, for two reasons that are not about
effort.** A device screenshot of the owner's phone would publish their training
history into a public README. And every golden renders with
`MediaRef.Unavailable`, so what appears is this app's placeholder icon rather
than a frame of Gym visual's artwork - a phone screenshot of a real workout
would have put licensed pixels in the repository (§6). Pointing at the goldens
also means the README cannot quietly show a version of the app that no longer
exists.

The first version padded every tile up to the tallest so a row would share a
baseline. It put the dead space straight back into exactly the screens the trim
had just rescued, which was visible in one look at the output and in no amount
of reasoning about it beforehand. Tiles keep their own heights now.

Today was dropped from the set. Trimmed it is 562px against the others' ~1080,
